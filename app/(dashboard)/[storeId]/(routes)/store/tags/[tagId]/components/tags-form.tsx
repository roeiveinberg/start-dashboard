...
type TagsFormValues = z.infer<typeof formSchema>;
...
export const TagsForm: React.FC<TagsFormProps> = ({
    ...
    const toastMassage = initialData ? "tag updated" : "tag created"
    ...
    const onSubmit = async (data: TagsFormValues) => {
        try {
            ...
            router.refresh()
            router.push(`/${params.storeId}/store/tags`)
    ...
    const onDelete = async () => {
        try {
            ...
            toast.error("Make sure you removed all products using this tag first")
        } finally {
            setLoading(false)
            setOpen(false)
        }
                                ...
                                <FormField
                                    ...
                                    render={({ field }) => (
                                        <FormItem>
                                            ...
                                            <FormControl>
                                                <Input disabled={loading} className="w-[293px]" placeholder="tag name" {...field} />
                                            </FormControl>
                                            <FormMessage />
                                        </FormItem>
                                    )}
